...
/*
 * Slightly modified by Robert Figura for use with teslawm
 */
...
char version[] = "@(#) vmenu version 0.1.8";
...
main(int argc, char **argv)
{
	...
	for(i = 0; 2*i+1 < numitems; ++i) {
	    commands[i] = argv[2*i];
	    labels[i] = argv[2*i+1];
	    if(strcmp(labels[i], initial) == 0)
		cur = i;
	    if(strlen(labels[i]) > maxlabel) {
		if(maxlabel > 3) 
		    labels[i][maxlabel-3] =
		    labels[i][maxlabel-2] =
		    labels[i][maxlabel-1] = '.';
		labels[i][maxlabel] = '\000';
	    }
	}
	numitems = numitems/2;
...
usage(void)
{
	...
	fprintf(stderr, "[-version] command title ...\n");
	exit(0);
}
...
run_menu(void)
{
	...
	for(;;) {
		...
		case ButtonPress:
		case MotionNotify:
			newcur = ev.xbutton.y/high;
			if(ev.xbutton.x < 0 || ev.xbutton.x > wide || newcur < 0 || newcur >= numitems)
			        break;
			if(cur == newcur)
				break;
			cur = newcur;
